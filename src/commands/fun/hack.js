...
exports.run = async (client, message, args) => {
    if (!args[0]) return client.sender(message, "401: No content.", "Nie podano argumentów. UwAżAj!", client.footer, "RED", "", "")

    const ip1 = Math.floor(Math.random() * (200 - 0) + 0)
    const ip2 = Math.floor(Math.random() * (201 - 0) + 0)
    const ip3 = Math.floor(Math.random() * (202 - 0) + 0)
    const ip4 = Math.floor(Math.random() * (203 - 0) + 0)
