    ...
    ws.on("message", function(msg) {
        ...
        switch (msg.t) {
            case "MESSAGE_CREATE":
                ...
                if (data.content.startsWith(prefix)) {r.db("users").table("users").filter({user_id: data.author.id}).run(client.con, (err,result) => {if (result._responses[0] && result._responses[0].r[0].gban) return console.log('test')})}
